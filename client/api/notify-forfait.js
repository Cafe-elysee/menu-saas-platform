...
// Prix de référence (identiques à control-app et cron-auto)
const PRICE = {
...
function buildForfaitEmail(lang, name, oldForfait, newForfait, isUpgrade, paymentMode) {
  ...
  const T = {
    ...
    en: {
      ...
      intro: isUpgrade ? `Your plan has been upgraded to <strong>${newLabel}</strong> (${modeLabel} — €${price}/month).` : `Your plan has been changed to <strong>${newLabel}</strong> (${modeLabel} — €${price}/month).`,
    ...
    el: {
      ...
      intro: isUpgrade ? `Η συνδρομή σας αναβαθμίστηκε σε <strong>${newLabel}</strong> (${modeLabel} — ${price}€/μήνα).` : `Η συνδρομή σας άλλαξε σε <strong>${newLabel}</strong> (${modeLabel} — ${price}€/μήνα).`,
    ...
    de: {
      ...
      intro: isUpgrade ? `Ihr Plan wurde auf <strong>${newLabel}</strong> aktualisiert (${modeLabel} — ${price}€/Monat).` : `Ihr Plan wurde auf <strong>${newLabel}</strong> geändert (${modeLabel} — ${price}€/Monat).`,
    ...
    ar: {
      ...
      intro: isUpgrade ? `تمت ترقية خطتك إلى <strong>${newLabel}</strong> (${modeLabel} — ${price}€/شهر).` : `تغيّرت خطتك إلى <strong>${newLabel}</strong> (${modeLabel} — ${price}€/شهر).`,
...
module.exports = async function handler(req, res) {
  ...
  // 2. Si upgrade : mettre à jour nextReminderAt dans Firebase contrôle
  if (isUpgrade) {
        ...
        if (commandes) {
          ...
          if (entry) {
            ...
            await fbPatch(CONTROL_DB, '/commandes/' + cmdKey, secret, {
              nextReminderAt: Date.now() + 7 * 24 * 3600 * 1000,
